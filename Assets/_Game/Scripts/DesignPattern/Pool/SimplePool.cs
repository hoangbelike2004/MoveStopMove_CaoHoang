...
{
    private static Dictionary<PoolType_One,Pool_One> poolInstance = new Dictionary<PoolType_One,Pool_One>();
    ...
    //lay phan tu
    public static T Spawn<T>(PoolType_One type,Vector3 pos, Quaternion rot) where T : GameUnit
    {
    ...
    // thu thap 1 phan tu
    public static void Collect(PoolType_One poolType)
    {
    ...
    //xoa 1 phan tu
    public static void ReLease(PoolType_One poolType)
    {
        if (!poolInstance.ContainsKey(poolType))
        {
            Debug.LogError($"{poolType} is not preLoad!!");
